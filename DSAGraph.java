...
public class DSAGraph implements Serializable {
	...
	private class DSAGraphVertex implements Serializable {
		...
		private DSAGraphVertex(String pLabel, Object pValue) {
			...
			value = pValue;
			edge = null;			//constructor
		}

		private void addEdge(edgeClass pEdge) {
			link.insertLast(pEdge);
			edge = pEdge;				//adds edge to the vertex's 'link' LinkedList and to the edge class field
			edgeCount++;
		...
		private void removeEdge(edgeClass edge) {
			for (Object o : link) {
				if (((edgeClass) o) == edge) {
					link.remove(edge);		//iterates through LinkedList of edges of the vertex and removes edge using remove function
				} else {
					throw new IllegalArgumentException("Edge does not exist");//throws exception if not found
				}
			}
		}
	...
	public void addEdge(double pWeight, String labelOne, String labelTwo, String pDirection, String pSecurity,
			String pBarriers) {			//adds edge to vertexex using imported parameters
	...
	public edgeClass[] bubbleSort(edgeClass[] arr) {
		int n = arr.length;
		edgeClass temp = null;
		for (int i = 0; i < n; i++) {
			for (int j = 1; j < (n - i); j++) {
				if (arr[j - 1].weight > arr[j].weight) {	//bubblesort method from practical 1 DSA
															//used to sort array by distance 
					// swap elements
					temp = arr[j - 1];
					arr[j - 1] = arr[j];
					arr[j] = temp;
	...
	void findEdge(String from, String to, double dis, String obst, String sec) {
		int count = 0;
		for (Object o : vertices) {
	...
	public void save(DSAGraph objToSave, String filename) {
		...
		try {
			...
			throw new IllegalArgumentException("Unable to save object " + e);
		}
	}
	...
	public DSAGraphVertex getVertex(String label) {
		...
		if (hasVertex(label) == false) {	//iterates through vertices list and returns matching vertex
			addVertex(label, 0);
		}
		for (Object o : vertices) {
			if (((DSAGraphVertex) o).label.equals(label)) {
		...
		for (Object o : pList) {
			...
			for (Object p : list) { // iterating through edges
				...
				{	//if edge is valid, inserted into linkedlist
					returnList.insertLast((edgeClass) p);
				}
	...
	public String[] sorter(LinkedList list, int print) {
		...
		bubbleSort(array); // sorts array by distance using bubblesort
